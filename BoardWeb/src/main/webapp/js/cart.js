...
let basket = {
		...
		svc.cartList(
			result => {
				...
				result.forEach(cart => {
					...
					rowDiv.querySelector('div.updown input').setAttribute('name', 'p_num' + cart.no);
		...
		svc.cartRemove(no,
			result => {
				if (result.retCode == 'OK') {
					...
					let qty = document.querySelector('#p_num' + no).value; // 삭제후 현재수량
					// 합계반영
	...
	changePNum: function(no) {
		...
		let price = document.querySelector('#p_price' + no).value;// 이벤트 누른 놈의 금액.
